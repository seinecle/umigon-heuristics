...
import static net.clementlevallois.umigon.model.heuristics.ConditionalExpression.ConditionEnum.isImmediatelyFollowedByAnOpinion;
...
public class IsImmediatelyFollowedByAnOpinion {
    ...
    public static ResultOneHeuristics check(String text, String termOrig, int indexTerm, HeuristicsLoaderOnDemand heuristics) {
        ResultOneHeuristics resultOneHeuristics = new ResultOneHeuristics(isImmediatelyFollowedByAnOpinion, termOrig, indexTerm, TypeOfTokenEnum.NGRAM);
        try {
            ...
            if (nextTerms.length > 0) {
                ...
                boolean found = heuristics.getMapH1().keySet().contains(temp) || heuristics.getMapH2().keySet().contains(temp);
                if (found) {
